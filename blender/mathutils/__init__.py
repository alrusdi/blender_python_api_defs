...
class Vector:
   ...
   def angle(other, fallback=None):
      '''Return the angle between two vectors.
         ...
         @fallback (any): return this value when the angle can't be calculated(zero length vector)
   ...
   def slerp(other, factor, fallback=None):
      '''Returns the interpolation of two unit vectors (spherical coordinates).
         ...
         @other (Vector): value to interpolate with.
         @factor (float): The interpolation value in [0.0, 1.0].
         @fallback (any): return this value when the vector can't be calculated(zero length vector or direct opposites)
